...
`notebooks/sanskrit_retrieval_finetune.ipynb` runs the whole thing (data → train → eval → RAG demo)
and **persists all outputs to Google Drive**.

1. Upload this folder to Google Drive.
2. Open the notebook in Colab → **Runtime → GPU (T4)**.
3. In cell 1, set `PROJECT_DIR` to the uploaded folder's path (e.g. `/content/drive/MyDrive/sanskrit-assignment`).
...
`notebooks/sanskrit_retrieval_finetune_mark3.ipynb` is an **advanced, research-backed recipe**
(CachedGISTEmbedLoss + Matryoshka + a cross-encoder reranking stage) — optional, for stronger results.
...
├── notebooks/                  # the Colab notebook (+ advanced mark3)
...
├── scripts/                    # notebook generators + md→pdf
